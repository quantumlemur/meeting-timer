var ViewModel = function() {
	...
	function draggedSchedule(d) {
		...
		d.scheduledStart = self.timeScale.invert(d3.event.y)
	...
	function resizedBottom(d) {
		console.log(d3.event.y)
		d.scheduledEnd = self.timeScale.invert(d3.event.y)
		d3.select(this.parentNode).select('.eventRect')
	...
	var newEvent = function(d) {
		...
		data.push({
			id: id,
			scheduledStart: self.timeScale.invert(d3.event.y),
			scheduledEnd: self.timeScale.invert(d3.event.y) + 60*60,
			done: false,
			active: false,
			name: id,
			speaker: id,
	...
	var updateTimings = function() {
		...
		for (var i=0; i<data.length; i++) {
			...
			if (i>0) {
				...
				console.log(i, formatTime(e.scheduledStart), formatTime(data[i-1].scheduledEnd), diff/60)
				if (diff > 0) {
					console.log('updating time', e.scheduledStart/60)
